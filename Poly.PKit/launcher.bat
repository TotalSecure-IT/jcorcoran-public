...
if /i "%MODE%"=="ONLINE" (
    ...
    powershell.exe -ExecutionPolicy Bypass -NoProfile -Command "Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/TotalSecure-IT/jcorcoran-public/refs/heads/main/Poly.PKit/main.ps1' -OutFile '%~dp0init\main.ps1'"
    ...
    if /i "%MODE%"=="CACHED" (
        ...
        if exist "init\cached.ps1" (
            echo Launching cached.ps1 in CACHED mode
            powershell.exe -ExecutionPolicy Bypass -NoProfile -File "init\cached.ps1"
        ) else (
            echo cached.ps1 not found in init folder.
            pause
            exit /b 1
        )
    )
